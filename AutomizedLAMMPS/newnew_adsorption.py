...
class SimulationManager:
    ...
    def step_3_detect_styles(self):
        ...
        def get_param_count(section_name):
            found_section = False
            for i, line in enumerate(lines):
                if section_name in line: found_section = True; continue
                if found_section:
                    clean = line.strip()
                    if not clean: continue
                    if clean[0].isalpha(): return 0
        ...
        if n_improper == 2: self.styles["improper"] = "umbrella" 
        elif n_improper == 3: self.styles["improper"] = "fourier"
        elif n_improper == 0: self.styles["improper"] = "umbrella" 
    ...
    def step_5_script(self, pressures):
        ...
        # === ИСПРАВЛЕННЫЙ БЛОК ГЕНЕРАЦИИ GCMC ===
        # 1. Сначала определяем фикс интеграции (движения)
        if self.props["rigid"]:
            # Используем ID 'myint'
            fix_integrator = f"fix myint gas rigid/nvt/small molecule temp ${{T}} ${{T}} 100.0"
            # Ссылаемся на него в ключевом слове 'rigid'
            gcmc_keyword_rigid = "rigid myint" 
        else:
            fix_integrator = f"fix myint gas nvt temp ${{T}} ${{T}} 100.0"
            gcmc_keyword_rigid = ""
        
        kw = f"mol gas_mol pressure ${{P_atm}} fugacity_coeff 1.0 {gcmc_keyword_rigid}"
        
        seed = np.random.randint(10000, 99999)
        
        # 2. Записываем в правильном порядке: сначала Fix Integrator, потом Fix GCMC
        script.append(f"    {fix_integrator}")
